...
description: Toggles on the Sync Behaviour card — sync_unpaid_invoices, sync_zero_value_invoices, sync_since, website_filter, store_filter. What each does, when to flip them.
---
...
When `true`: every invoice — paid or unpaid — posts to Xero on creation. The separate `invoice.paid` event later fires `POST /Payments` to mark it PAID when Magento marks it paid.
...
Magento occasionally creates £0 invoices for fully-discounted orders, free samples, gift-card-only purchases, or stock-adjustment scenarios. When `false`, these are skipped (`reason: zero_value_invoice`) — useful for keeping the Xero AR ledger clean of "noise" lines.
